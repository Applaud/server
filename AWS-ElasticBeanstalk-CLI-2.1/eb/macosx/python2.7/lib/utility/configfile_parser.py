...
class SectionedConfigParser(_RawConfigParser):
    ''' 
    Derived ConfigParser.RawConfigParser to support Unicode write 
    '''
    
    OPTCRE_NV = _re.compile(
    ...
    def read(self, pathfilename):
        with _codecs.open(pathfilename, 'r', encoding=ServiceDefault.CHAR_CODEC) as input_file:
            _RawConfigParser.readfp(self, input_file)

    def write(self, pathfilename):
        with _codecs.open(pathfilename, 'w', encoding=ServiceDefault.CHAR_CODEC) as output_file:
            for section in self._sections:
                output_file.write(u'[{0}]{1}'.format(section, _os.linesep))
                for (option, value) in self._sections[section].items():
                    if option == "__name__":
                        continue
                    output_file.write(u'{0} = {1}{2}'.format(option, value, _os.linesep))
                output_file.write(_os.linesep)
...
class NoSectionConfigParser(_RawConfigParser):
    ...
    def write(self, pathfilename):
        with _codecs.open(pathfilename, 'w', encoding='utf-8') as output_file:
            for option in self.options(self._default_section):
                output_file.write(u'{0} = {1}{2}'.format(option, self.get(option), _os.linesep))
